...
## 8. c++ const(常量引用, 常对象，和对象的常成员)用法详解
> 常引用
```cpp
...
2. 常对象不能调用普通的成员函数，没有对外的接口，需要专门为常对象定义常成员函数
    ...
    b. 常成员函数不能修改对象的数据成员(而通过指针维护的对象确实可以修改的)，也不能访问类中没有const声明的非常成员函数
    c. 常对象只能调用它的常成员函数，不能调用其他的普通成员函数
   	d. const关键字可以被用于参与对重载函数的区分(只有是底层的const, 顶层的const会被忽略不能用于重载)
    e. 非const对象可以访问任意的成员函数，包括const成员函数；
    f. 构造函数和析构函数不能是常函数(构造进行初始化，析够进行释放，这些操作都会使成员发生改变)
...
>浅谈std::forward
```cpp
std::forward通常是用于完美转发的，它数的也是右值。一个经典的完美转发的场景是：
...
## 16 const和constexpr的区别

>const

```cpp
1. const并不能代表“常量”，它仅仅是对变量的一个修饰，告诉编译器这个变量只能被初始化，且不能被直接修改（实际上可以通过堆栈溢出等方式修改）。而这个变量的值，可以在运行时也可以在编译时指定。
```

> constexpr

```cpp
1. constexpr可以用来修饰变量、函数、构造函数。一旦以上任何元素被constexpr修饰，那么等于说是告诉编译器 “请大胆地将我看成编译时就能得出常量值的表达式去优化我”。
2. constexpr是C++11中新增的关键字，其语义是“常量表达式”，也就是在编译期可求值的表达式。最基础的常量表达式就是字面值或全局变量/函数的地址或sizeof等关键字返回的结果
3. 编译器可以在编译期对constexpr的代码进行非常大的优化，比如将用到的constexpr表达式都直接替换成最终结果等;
4. 相比宏来说，没有额外的开销，但更安全可靠。
5. constexpr还能用于修饰类的构造函数，即保证如果提供给该构造函数的参数都是constexpr，那么产生的对象中的所有成员都会是constexpr，该对象也就是constexpr对象了
...
//如果foo传入的参数是常量表达式，则返回的是常量表达式，否则就不是返回常量表达式
...
```cpp
1. const是变量类型名的一部分，即 part of type name，一个名字叫“const T”或者“T const”的类型，和T这个类型本身处于一种平级的关系，和T不同的就在于这个类型的对象自产生后就不能再更改了;
2. constexpr是声明的一部分，即 part of a declaration，他不是变量类型的一部分。当他出现在一个变量的声明中时，他要求编译器在编译期间就初始化并确定下来这个变量（否则编译报错）；当他出现在一个函数的声明中时，他要求至少有一条return路径可以（但不是必须）在编译中确定下来，即返回的是编译期常量
3. 二者的联系就在于，在使用constexpr声明一个类型为T的变量时，constexpr会自动把这个变量定义为const T类型。即constexpr在完成它本职工作（告诉编译器这是个编译期常量）的同时，还把原来的T类型改为了const T类型。这就是二者的联系
```
