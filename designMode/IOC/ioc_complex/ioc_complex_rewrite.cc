...
public:
    template <typename U>
    std::shared_ptr<T> getRegisterSharedPtr(std::string key) {
        if (KeyValueMap.find(key) == KeyValueMap.end()) {
            RegisterType<U>(key);
        }
        return ResolveShared(key);
    }
...
int main () {
    ...
    auto DeviceB = Ioc.getRegisterSharedPtr<B>("B");
    auto DeviceC = Ioc.getRegisterSharedPtr<C>("C");
    DeviceB->fun_print();
    DeviceC->fun_print();
